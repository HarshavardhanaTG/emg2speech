...
class SlidingCovariance:
    """Computes band-wise covariance matrices over sliding time windows,
    and optionally transforms them using provided eigenvectors.

    Input: (T, 2, 16)  
    Output: (num_windows, 2, 16, 16)
    """

    eigenvectorsL: torch.Tensor
    eigenvectorsR: torch.Tensor
    window_size: int = 32
    step_size: int = 16
    alpha: float = 0.9
    eps: float = 1e-5
    ...
    def __post_init__(self):
        ...
        assert self.eigenvectorsR.shape == (16, 16)

    def sliding_window_view(self, x: torch.Tensor, window_size: int, step_size: int):
        T, C = x.shape
        num_windows = (T - window_size) // step_size + 1
        shape = (num_windows, window_size, C)
        strides = (x.stride(0) * step_size, x.stride(0), x.stride(1))
        return x.as_strided(shape, strides)

    def compute_band_covariances(self, band: torch.Tensor, eig: torch.Tensor) -> torch.Tensor:
        # Get sliding windows: (num_windows, 2*window_size, 16)
        windows = self.sliding_window_view(band, 2 * self.window_size, self.step_size)

        # Compute covariance: (num_windows, 16, 16)
        covs = torch.matmul(windows.transpose(-1, -2), windows) / (2 * self.window_size)

        # Regularization
        traces = covs.diagonal(dim1=-2, dim2=-1).sum(-1)  # (num_windows,)
        eye = torch.eye(covs.shape[-1], device=covs.device).expand(covs.shape[0], -1, -1)
        covs = self.alpha * covs + (1 - self.alpha) * traces[:, None, None] * eye

        # Apply eigenvector transform only if approxDiag is True
        if self.approxDiag:
            covs = torch.matmul(eig.T, torch.matmul(covs, eig))

        return covs  # (num_windows, 16, 16)

    def __call__(self, tensor: torch.Tensor) -> torch.Tensor:
        assert tensor.ndim == 3 and tensor.shape[1] == 2 and tensor.shape[2] == 16, \
            f"Expected shape (T, 2, 16), got {tensor.shape}"

        band0 = tensor[:, 0, :]  # (T, 16)
        band1 = tensor[:, 1, :]  # (T, 16)

        covs0 = self.compute_band_covariances(band0, self.eigenvectorsL)
        covs1 = self.compute_band_covariances(band1, self.eigenvectorsR)

        # Stack along band dim -> (num_windows, 2, 16, 16)
        return torch.stack([covs0, covs1], dim=1)


@dataclass
class FlattenSPDSequence:
    """Flattens a (num_windows, 2, 16, 16) tensor into (num_windows, 512).
    
    Assumes:
        - num_windows: number of sliding covariance windows
        - 2 bands
        - 16 channels per band
    """

    def __call__(self, tensor: torch.Tensor) -> torch.Tensor:
        assert tensor.ndim == 4 and tensor.shape[1:3] == (2, 16), \
            f"Expected shape (num_windows, 2, 16, 16), got {tensor.shape}"
        
        return tensor.view(tensor.shape[0], -1)  # (num_windows, 512)
    

@dataclass
class ZNormalizeTime:
    """Z-normalizes the tensor along the time dimension (dim=0).
